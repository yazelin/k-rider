...
- 名單看後台：開 `admin.html`，貼 `ADMIN_TOKEN`（存瀏覽器 localStorage），頁面有「訂閱／報名／回饋」三個分頁——訂閱打 `GET /admin/list`、報名打 `GET /admin/registrations`（可下拉篩梯次）、回饋打 `GET /admin/notes`（可下拉篩種類：想聽這個／想來講／心得），各自匯出 CSV。後台頁 `noindex`，不進搜尋引擎。
